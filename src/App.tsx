...
const STORAGE_KEY = 'borrow-system-student-affairs-v5';
...
function makeBorrowCode(total) {
  const date = new Date().toISOString().slice(0, 10).replaceAll('-', '');
  return `BR-${date}-${String(total + 1).padStart(3, '0')}`;
}
...
export default function App() {
  ...
  const filteredRequests = useMemo(() => {
    return displayRequests.filter((request) => {
      ...
      const text =
        `${request.code} ${request.fullname} ${request.email} ${request.phone} ${request.department} ${request.purpose} ${itemText}`.toLowerCase();
      const okSearch = text.includes(search.toLowerCase());
      const okStatus =
        statusFilter === 'ทั้งหมด' || request.displayStatus === statusFilter;
  ...
  const myRequests = displayRequests.filter(
    (request) =>
      emailSearch &&
      request.email.toLowerCase().includes(emailSearch.toLowerCase())
  );

  const stats = useMemo(() => {
    return {
      total: displayRequests.length,
      pending: displayRequests.filter((r) => r.displayStatus === STATUS.pending)
        .length,
      approved: displayRequests.filter(
        (r) => r.displayStatus === STATUS.approved
      ).length,
      overdue: displayRequests.filter((r) => r.displayStatus === STATUS.overdue)
        .length,
      returned: displayRequests.filter(
        (r) => r.displayStatus === STATUS.returned
      ).length,
      rejected: displayRequests.filter(
        (r) => r.displayStatus === STATUS.rejected
      ).length,
    };
  }, [displayRequests]);
  ...
  function addItem() {
    setForm({
      ...form,
      items: [...form.items, { name: '', qty: 1, note: '', photo: '' }],
    });
  }
  ...
  async function submitRequest(event) {
    ...
    if (form.borrowDate > form.dueDate)
      return alert('วันที่คืนต้องไม่น้อยกว่าวันที่ยืม');
    if (form.items.some((item) => !item.name.trim()))
      return alert('กรุณากรอกรายการของที่ยืมให้ครบ');
    if (form.items.some((item) => Number(item.qty) < 1))
      return alert('จำนวนต้องมากกว่า 0');
    if (!form.agree) return alert('กรุณายืนยันเงื่อนไขการยืม');

    const newRequest = {
      id: Date.now(),
      code: makeBorrowCode(requests.length),
      ...form,
...
เลขที่คำขอ: ${newRequest.code}`);
  ...
  async function confirmApprove() {
    ...
    setRequests((old) =>
      old.map((request) =>
        request.id === approveTarget.id ? approvedRequest : request
      )
    );
  ...
  async function rejectRequest(request) {
    ...
    setRequests((old) =>
      old.map((item) => (item.id === request.id ? rejectedRequest : item))
    );
  ...
  async function saveReturn() {
    ...
    if (returnChecklist.some((item) => !item.returned))
      return alert('กรุณาติ๊กคืนครบทุกชิ้นก่อนบันทึกคืน');
    ...
    setRequests((old) =>
      old.map((request) =>
        request.id === returnTarget.id ? returnedRequest : request
      )
    );
            ...
            <section className="card">
              ...
              <form onSubmit={submitRequest} className="form">
                ...
                <div className="grid2">
                  <Field label="วันที่ยืม">
                    <input
                      ...
                      onChange={(e) => updateForm('borrowDate', e.target.value)}
                      required
                    />
                  </Field>
                    ...
                    <div className="itemBox" key={index}>
                      ...
                      <Field label="หมายเหตุ">
                        <input
                          ...
                          placeholder="เช่น สีดำ / ขนาดใหญ่"
                        />
                      </Field>
                      {form.items.length > 1 && (
                        <button
                    ...
                    <div className="photoCard" key={index}>
                      ...
                      <label className="uploadBox smallUpload">
                        ...
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => handleItemPhoto(index, e)}
                          hidden
                        />
                ...
                <label className="agree">
                  ...
                  />
                  <span>
                    ข้าพเจ้ายืนยันว่าจะดูแลรักษาวัสดุ/ครุภัณฑ์ และคืนตามกำหนด
                    หากชำรุดหรือสูญหายจะแจ้งเจ้าหน้าที่ทันที
                  </span>
                </label>
            ...
            <aside className="rightCol">
              <section className="card">
                ...
                <input
                  ...
                  placeholder="กรอกอีเมลเพื่อดูสถานะ"
                  value={emailSearch}
                  onChange={(e) => setEmailSearch(e.target.value)}
                />
                <div className="statusList">
                  {myRequests.length === 0 && (
                    <p className="empty">กรอกอีเมลเพื่อดูสถานะ</p>
                  )}
                  {myRequests.map((request) => (
                    ...
                    >
                      ...
                      <small>
                        ยืม {request.borrowDate} • คืน {request.dueDate}
                      </small>
                    </button>
                  ))}
                </div>
          ...
          <main className="adminPage">
            ...
            <section className="card">
              ...
              <div className="filterBar">
                <input
                  placeholder="ค้นหาชื่อ / อีเมล / เบอร์ / รายการ"
                  value={search}
        ...
        {returnTarget && (
          <div className="modalBackdrop">
            <div className="modal smallModal">
              ...
              <p>
                <b>ผู้ยืม:</b> {returnTarget.prefix}
                {returnTarget.fullname}
              </p>
              <p>
                <b>เช็กลิสต์รายการคืน:</b>
              </p>
              <div className="returnChecklist">
                {returnChecklist.map((item, index) => (
                  <div className="returnCheckItem" key={index}>
                    ...
                    {item.note && (
                      <p className="muted">หมายเหตุเดิม: {item.note}</p>
                    )}
                    <div className="grid2 returnGrid">
                      ...
                      <Field label="หมายเหตุการคืน">
                        <input
                          ...
                          placeholder="เช่น ครบ / มีรอย / ขาดอุปกรณ์"
                        />
                      </Field>
                    </div>
                  </div>
        ...
        {showClearConfirm && (
          <div className="modalBackdrop">
            <div className="modal smallModal">
              ...
              <p>ต้องการล้างข้อมูลคำขอทั้งหมดใช่ไหม?</p>
              <div className="actionRow">
                <button
                  type="button"
                  className="btn danger"
  ...
  return (
    <div className="requestCard">
      <div className="requestHeader">
        <div>
          ...
          <p>
            ...
            {request.department} • ชั้นปี {request.year}
          </p>
        </div>
        {request.items.some((item) => item.photo) && (
          <div className="photoPreviewGroup">
            {request.items.map((item, index) =>
              item.photo ? (
                <img key={index} src={item.photo} alt={item.name} />
              ) : null
            )}
          </div>
        )}
      ...
      <div className="borrowItems">
        {request.items.map((item, index) => (
          <div key={index}>
            • <b>{item.name}</b> จำนวน <b>{item.qty}</b>
      ...
      <p className="muted">
        งาน: {request.purpose} • ยืม {request.borrowDate} • กำหนดคืน{' '}
        {request.dueDate}
      </p>
      {request.approvedBy && (
        <p className="blue">ผู้อนุมัติ: {request.approvedBy}</p>
      )}
      {request.returnedAt && (
        <p className="blue">
          คืนแล้ว: {request.returnedAt} • ผู้รับคืน: {request.returnedBy}
        </p>
      )}
      {request.returnChecklist && request.returnChecklist.length > 0 && (
        <div className="returnSummary">
          {request.returnChecklist.map((item, index) => (
            <div key={index}>
              ✓ {item.name} จำนวน {item.qty} • {item.condition}
              {item.returnNote ? ` (${item.returnNote})` : ''}
            </div>
          ))}
        </div>
      ...
      {request.adminNote && (
        <p className="red">หมายเหตุ: {request.adminNote}</p>
      )}
      ...
      <div className="actionRow">
        ...
        {request.displayStatus === STATUS.pending && (
        ...
        {(request.displayStatus === STATUS.approved ||
          request.displayStatus === STATUS.overdue) && (
          <button type="button" className="btn primary" onClick={onReturn}>
            บันทึกคืน
          </button>
        )}
        <button type="button" className="btn danger light" onClick={onDelete}>
          ลบ
        </button>
      </div>
    </div>
    ...
    <div className="modalBackdrop">
      <div className="modal">
        ...
        <div className="detailGrid">
          <p>
            <b>ผู้ยืม:</b> {request.prefix}
            {request.fullname}
          </p>
          <p>
            <b>อีเมล:</b> {request.email}
          </p>
          <p>
            <b>เบอร์โทร:</b> {request.phone || '-'}
          </p>
          <p>
            <b>สาขา/ฝ่าย:</b> {request.department}
          </p>
          <p>
            <b>ชั้นปี:</b> {request.year}
          </p>
          <p>
            <b>วัตถุประสงค์:</b> {request.purpose}
          </p>
          <p>
            <b>วันที่ยืม:</b> {request.borrowDate}
          </p>
          <p>
            <b>กำหนดคืน:</b> {request.dueDate}
          </p>
          <p>
            <b>ผู้อนุมัติ:</b> {request.approvedBy || '-'}
          </p>
          <p>
            <b>ผู้รับคืน:</b> {request.returnedBy || '-'}
          </p>
          <p>
            <b>วันที่คืน:</b> {request.returnedAt || '-'}
          </p>
        </div>
        <h3>รายการของที่ยืม</h3>
        {request.items.map((item, index) => (
          <div className="modalItem" key={index}>
            {index + 1}. {item.name} จำนวน {item.qty}{' '}
            {item.note ? `(${item.note})` : ''}
          </div>
        ))}
        {request.returnChecklist && request.returnChecklist.length > 0 && (
          <>
            <h3>เช็กลิสต์การคืน</h3>
            {request.returnChecklist.map((item, index) => (
              <div className="modalItem" key={index}>
                ✓ {index + 1}. {item.name} จำนวน {item.qty} • {item.condition}
                {item.returnNote ? ` (${item.returnNote})` : ''}
              </div>
            ))}
          </>
